...
def main():
    ...
    #             lamb = 0.0
    #             cross_validation(model, transfer_data, valid_data, matrix_size, lr, lamb, num_epoch, splits[1])
    ...
    # accuracies = cross_validation(model, transfer_data, valid_data, matrix_size, 0.01, lamb_set[1],
    #                               25, splits[1])
    lamb_values = [0.001, 0.01, 0.1, 1]
    max_accuracy, optimal_lamb = 0, 0

    for lamb in lamb_values:
        model = AutoEncoder(num_question=train_matrix.shape[1], k=10)
        acc = cross_validation(model, transfer_data, valid_data, matrix_size, 0.01, lamb_set[1],
                         25, splits[1])
        valid_accuracy = evaluate(model, zero_train_matrix, valid_data)
        if valid_accuracy > max_accuracy:
            max_accuracy = valid_accuracy
